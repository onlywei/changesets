...
async function publishPackages({
  ...
  logger.error('!!! unpublishedPackagesInfo.length', unpublishedPackagesInfo.length);
...
async function getUnpublishedPackages(packages, preState) {
  const results = await Promise.all(packages.map(async ({
    ...
    logger.error('!!! Getting info for package', packageJson.name);
    ...
    logger.error('!!! Got info for package', infoResult);
    return infoResult;
  }));
